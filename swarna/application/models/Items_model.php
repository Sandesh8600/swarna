    ...
    class Items_model extends CI_Model{
      ...
      */
      public function fetch_user($user_id){
        ...
        $query=$this->db->query("select u.* from items u where u.Item_Code=?", array($user_id));
      ...
      */
      public function get_users($page_start,$page_size){
		...
		//  }
		 ...
		 $user_fields=array("Item_Code","Item_Name","Status","Timestamp");
        ...
        //count total 
        $count_query=$this->db->query("select count(u.Item_Code) as total_records from items u where 1 ".$where, $count_params);
        $count_results=$count_query->row_array();
        
        //build query
        $query=$this->db->query("select u.* from items u where 1 ".$where.$sort_query." limit ?,?",$params);
       ...
       public function update(){
		 ...
		 $c_date = date("Y-m-d H:i:s");
		  ...
		  if($this->input->post("Password")){
		   ...
		   $this->db->update("items", $values, array("Item_Code"=>$user_id));
