<?php
    class Polish_model extends CI_Model{
      ...
      */
      public function fetch_user($user_id){
        ...
        $query=$this->db->query("select u.*,pricebreakup.* from polish_orders u join pricebreakup ON pricebreakup.Order_Code = u.Order_Code where u.Order_Code=?", array($user_id));
      ...
      */
      public function get_users($page_start,$page_size,$order_type=""){
        ...
        $count_query=$this->db->query("select count(u.Order_Code) as total_records from polish_orders u join customers ON customers.Customer_Code = u.Customer_Code where 1 ".$where, $count_params);
		
        $count_results=$count_query->row_array();
        
        //build query
		$query=$this->db->query("select u.*, customers.Customer_Name, customers.Customer_Mobile_Number1, (select sum(making_charges) as total_mc from polish_items where Order_Code=u.Order_Code) as total_items,  (select sum(approx_grams+(approx_grams*wastage/100)) as total_og from polish_items where Order_Code=u.Order_Code) as total_og, (select sum(((Grams-melting_loss)*Quality/100)+ROUND(((Grams-melting_loss)*Quality/100)*copper/100, 3)) as total_grams from polish_payments where Order_Code=u.Order_Code ) as total_grams from polish_orders u join customers ON customers.Customer_Code = u.Customer_Code where 1 ".$where.$sort_query."  ",$params);
        //$query=$this->db->query("select u.*, customers.Customer_Name, customers.Customer_Mobile_Number1, (select count(id) from polish_items where Order_Code=u.Order_Code) as total_items, (select count(id) from polish_items where Order_Code=u.Order_Code and Workshop_Code>0) as total_items_assigned from polish_orders u join customers ON customers.Customer_Code = u.Customer_Code where 1 ".$where.$sort_query."  ",$params); //query bef total charges etc
       ...
       public function update(){
		 ...
		 $c_date = date("Y-m-d H:i:s");
		  ...
		  if($this->input->post("Password")){
		   ...
		   $this->db->update("polish_orders", $values, array("Order_Code"=>$user_id));
	 ...
	 public function get_subcats_by_cat($cat_id){
	  ...
	  public function create_new_order(){
		...
		$order_type = $this->input->post("order_type");
			...
			$this->db->insert("polish_orders", $values);
			...
			$this->db->update("polish_orders", array('orderid'=>$order_id));
			if($Order_Code){
				...
				// 	$this->db->insert_batch("polish_items", $items);
				...
				if($cash_receipts){
					...
					foreach($cash_receipts as $key=>$val):
						...
						$this->db->insert("polish_payments", $val);
		...
		public function add_order_items($Order_Code,$val){	
			if($Order_Code){
				...
				$this->db->insert("polish_items", $val);
	  ...
	  public function update_new_order(){
			...
			$this->db->update("polish_orders", $values, array("Order_Code"=>$order_code));
			
			
			if($order_code){
				
				//remove old items
				$this->db->delete("polish_items", array("Order_Code"=>$order_code));
				...
				//remove old receipts
				$this->db->delete("polish_payments", array("Order_Code"=>$order_code));
	  ...
	  public function get_order($order_id){
			...
			//get order payments
			$query2=$this->db->query("select * from polish_payments where Order_Code=?", $order_id);
