<?php
    class Viewtask_model extends CI_Model{
      ...
      */
      public function fetch_user($user_id){
        ...
        $query=$this->db->query("select u.* from taskstoworkshop u where u.Task_Id=?", array($user_id));
      ...
      */
      public function get_users($page_start,$page_size){
		...
		//  }
		 ...
		 $user_fields=array("Task_Id","Task_Title","Timestamp");
        ...
        //count total 
        $count_query=$this->db->query("select count(u.Task_Id) as total_records from taskstoworkshop u where 1 ".$where, $count_params);
		...
		join tasktype ON tasktype.TaskType_Id = u.TaskType_Id where u.Order_code=".$this->session->userdata('viewtask_id')." and 1 ".$where.$sort_query." limit ?,?",$params);
       ...
       public function update(){
		 ...
		 $c_date = date("Y-m-d H:i:s");
		  ...
		  if($this->input->post("Password")){
		   ...
		   $this->db->update("taskstoworkshop", $values, array("Task_Id"=>$user_id));
