<?php
    class Jewellary_model extends CI_Model{
      ...
      */
      public function fetch_user($user_id){
        ...
        $query=$this->db->query("select u.* from gold_transaction u where u.Transaction_ID=?", array($user_id));
      ...
      */
      public function get_users($page_start,$page_size){
		...
		//  }
		 ...
		 $user_fields=array("Transaction_ID","Ticket_title","Timestamp");
        ...
        //count total 
        $count_query=$this->db->query("select count(u.Transaction_ID) as total_records from gold_transaction u where 1 ".$where, $count_params);
        $count_results=$count_query->row_array();
        
        //build query
        // $query=$this->db->query("select u.* from gold_transaction u where 1 ".$where.$sort_query." limit ?,?",$params);
        $query=$this->db->query("select u.* from gold_transaction u order by u.Transaction_ID desc");
       ...
       public function update(){
		 ...
		 $c_date = date("Y-m-d H:i:s");
		  ...
		  if($this->input->post("Password")){
		   ...
		   $this->db->update("gold_transaction", $values, array("Transaction_ID"=>$user_id));
