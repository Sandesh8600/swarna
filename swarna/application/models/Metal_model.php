    ...
    class Metal_model extends CI_Model{
      ...
      */
      public function fetch_user($user_id){
        ...
        $query=$this->db->query("select u.* from metalorpurity u where u.Purity_ID=?", array($user_id));
      ...
      */
      public function get_users($page_start,$page_size){
		...
		//  }
		 ...
		 $user_fields=array("Purity_ID","Purity_Name","Timestamp");
        ...
        //count total 
        $count_query=$this->db->query("select count(u.Purity_ID) as total_records from metalorpurity u where 1 ".$where, $count_params);
        $count_results=$count_query->row_array();
        
        //build query
        $query=$this->db->query("select u.* from metalorpurity u where 1 ".$where.$sort_query." limit ?,?",$params);
       ...
       public function update(){
		 ...
		 $c_date = date("Y-m-d H:i:s");
		  ...
		  if($this->input->post("Password")){
		   ...
		   $this->db->update("metalorpurity", $values, array("Purity_ID"=>$user_id));
