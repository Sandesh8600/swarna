<?php
    class Golddeposit_model extends CI_Model{
      ...
      */
      public function fetch_user($user_id){
        ...
        $query=$this->db->query("select u.* from golddepositeticket u where u.Ticket_Id=?", array($user_id));
      ...
      */
      public function get_users($page_start,$page_size){
		...
		//  }
		 ...
		 $user_fields=array("Ticket_Id","Ticket_title","Timestamp");
        ...
        //count total 
        $count_query=$this->db->query("select count(u.Ticket_Id) as total_records from golddepositeticket u where 1 ".$where, $count_params);
        $count_results=$count_query->row_array();
        
        //build query
        $query=$this->db->query("select u.*,workshops.*,taskstoworkshop.* from golddepositeticket u join workshops ON workshops.Workshop_Code = u.Workshop_Code 
		join taskstoworkshop ON taskstoworkshop.Task_Id = u.Task_Id where u.Order_code=".$this->session->userdata('viewtask_id')." and 1 ".$where.$sort_query." limit ?,?",$params);
       ...
       public function update(){
		 ...
		 $c_date = date("Y-m-d H:i:s");
		  ...
		  if($this->input->post("Password")){
		   ...
		   $this->db->update("golddepositeticket", $values, array("Ticket_Id"=>$user_id));
