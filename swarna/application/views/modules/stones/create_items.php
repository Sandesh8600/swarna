...
<div class="form-div">
	...
	<form action="<?php echo site_url("stones/browse?item_create_form=yes"); ?>" method='POST' enctype="multipart/form-data" name="create_form" id="create-form-id">
		
	  <div class="form-block">
	       <div class="form-row" style="width:100%;display:flex;">
				<div class="form-column">
					<label class="radio-label">Select Stone Type</label>
			        <select name="stone_type_id" class="form-select valid" required="" aria-invalid="false" id="stone_type_id" onchange='sub_category_options($(this).val());'>
				...
				<div class="form-column">
					<label class="radio-label">Select Stone Sub Type</label>
			        <select name="stone_subtype_id" class="form-select valid" required="" aria-invalid="false" id="stone_subtype_id">
						<option>Select Stone Sub Type</option>
	...
	function sub_category_options(category_id){
		...
		<?php
			...
			foreach($users as $cat):
				...
				$subcats="<option value=''>-Select Stone Sub Type-</option>";
			
				foreach($cat['stone_sub_types'] as $sub_cat):
				
					$subcats=$subcats."<option value='".$sub_cat['id']."'>".$sub_cat['name']."</option>";			
				endforeach;
			
				?>
				subcat_options[<?php echo $cat['id']; ?>]="<?php echo $subcats; ?>";
				<?php
			
			endforeach;
		?>
		
		$("#stone_subtype_id").html(subcat_options[category_id]);
