        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="<?php echo site_url("order/create_new_order?metal_type=$metal_type"); ?>"><button class="btn btn-primary">Create </button></a></li>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card">
				...
				<form class="browse-filter-form" action="<?php echo site_url("order/browse"); ?>">
			<div class="filter-form-row">
				...
				<div class="form-column">
					...
					<div>
						<input type="submit" class="btn btn-primary" value="Apply" /> <a href="<?php echo site_url("order/browse"); ?>" class="btn btn-primary bg-grey">&nbsp;<i class="fas fa-redo"></i> &nbsp; Reset</a>
					</div>
				</div>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card">
              <!-- <div class="card-header">
				...
				<div class="card-tools">
                  <ul class="pagination pagination-sm float-right">
				        <?php if($prev_page): ?>
							<li class="page-item"><a href="<?php echo site_url("order/browse?page=".$prev_page_num); ?>" class="page-link">&laquo;</a></li>
						<?php endif; ?>
						...
						<?php
								...
								if($total_pages>14 and $current_page==$total_pages){ $start=$current_page-10;  $total=($total+$start)>$total_pages?$total_pages:($total+$start); }
									...
									for($i=$start; $i<=$total; $i++){
										...
										if(($i)!=$current_page){
									?>
										<li class="page-item"><a href="<?php echo site_url("order/browse?page=".($i-1)."&".$query_string_pagination); ?>" class="page-link"><?php echo $i; ?></a></li>
									<?php
								 } else {
						...
						<?php if($next_page): ?>
							<li class="page-item"><a href="<?php echo site_url("order/browse?page=".$next_page_num); ?>" class="page-link">&raquo;</a></li>
						<?php endif; ?>
              ...
              <!-- /.card-header -->
              <div class="card-body">
                <table class="table table-bordered display" id="rate_table">
				<tfoot>
					<tr>
					<th colspan="4" style="text-align:right">Total Order Balance:</th>
				  ...
				  <?php foreach($users as $user):  ?>
						  <tr>
								...
								<td><?php echo ucfirst($user['metal_type']); ?></td>
								...
								<td align="right"><?php 
								 ...
								 $tgb = number_format((float)($user['total_grams']-$user['total_recipts']-$user['total_gram_recipts']), 3, '.', '');
								 echo convert_to_grams($tgb);
								 ?></td> 
							
								
								<td style="text-align:right"><?php echo number_format((float)($user['total_mc'] - $user['total_paid_mc']), 2, '.', ''); ?></td>
								<td align="right"><?php  echo number_format((float)($user['total_sc']-$user['total_paid_sc']), 2, '.', ''); ?></td>
								<td>
									<a href="<?php echo site_url("order/edit_new_order/".$user['Order_Code']); ?>" class="" title="Edit Membership"><i class="fas fa-pencil-alt" style=" font-size:1.3em; position:relative; top:2px; color:#ffb800;"></i></a>
									...
									<a href="<?php echo site_url("order/view_new_order/".$user['Order_Code']); ?>" style="color:green;cursor:pointer;margin-left:10px;"><i class="fas fa-eye trashclass" ></i></a>
...
<div class="module-title-section">
	...
	<a href="<?php echo site_url("order/create"); ?>" class="form-button small-button bg-green">Add New Order</a>
...
<br>
			...
			<div>
			 <input type="submit" class="form-button small-button bg-green" value="Apply" /> <a href="<?php echo site_url("order/browse"); ?>" class="form-button small-button bg-grey">&nbsp;<i class="fas fa-redo"></i> &nbsp; Reset</a>
			 </div>
		</form>
	</div>
</div>
...
<div class="module-content-section" style="width:100%;overflow:auto;" data-aos="fade-up" data-aos-delay="200">
	...
	<div class="table-container">
						<table>
						  <tr>
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=id&sort_type=".$sort_type); ?>">Order ID <?php if($sort_key=="id" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="id" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
								
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=firstname&sort_type=".$sort_type); ?>">Customer&nbsp;Name/ID <?php if($sort_key=="firstname" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="firstname" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=firstname&sort_type=".$sort_type); ?>">Product&nbsp;Name <?php if($sort_key=="firstname" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="firstname" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=firstname&sort_type=".$sort_type); ?>">Order Date <?php if($sort_key=="firstname" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="firstname" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=firstname&sort_type=".$sort_type); ?>">Required Date <?php if($sort_key=="firstname" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="firstname" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=firstname&sort_type=".$sort_type); ?>">Shipped Date <?php if($sort_key=="firstname" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="firstname" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=firstname&sort_type=".$sort_type); ?>">Order Status <?php if($sort_key=="firstname" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="firstname" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
								
								<th><a href="<?php echo site_url("order/browse?".$query_string_sort."&sort_key=parent_email&sort_type=".$sort_type); ?>">Time <?php if($sort_key=="parent_email" and $sort_type=="asc"): ?><i class="fas fa-sort-up"></i><?php elseif($sort_key=="parent_email" and $sort_type=="desc"): ?><i class="fas fa-sort-down"></i><?php endif; ?></a></th>
						  ...
						  <tr>
								...
								<td>
									<a href="<?php echo site_url("order/edit/".$user['Order_Code']); ?>" class="" title="Edit Membership"><i class="fas fa-pencil-alt" style=" font-size:1.3em; position:relative; top:2px; color:#ffb800;"></i></a>
									<a onclick="save(<?php echo $user['Order_Code']; ?>)" style="color:red;cursor:pointer;margin-left:10px;"><i class="fas fa-trash trashclass" ></i></a>
									
									<form action="<?php echo site_url("order/submitid"); ?>" method="post">
...
const save = async (partner_id) => {
  ...
  if (confirm) {
	$.post("<?php echo site_url("order/mydelete");?>",
       {partner_id:partner_id}, function(data){
           window.location.href="<?php echo site_url("order/browse?metal_type=$metal_type"); ?>";
       });
  } else {
    // alert("no clicked");
  }
...
function myDelete(partner_id)
  {
    //    alert(partner_id);
       $.post("<?php echo site_url("order/mydelete");?>",
       {partner_id:partner_id}, function(data){
           window.location.href="<?php echo site_url("order/browse"); ?>";
       });
  }
  ...
  $('#rate_table').DataTable( {
        ...
        "bPaginate": true,
			...
			var tgb = tgbapi.column(4).data().reduce(function (a, b) {
				var gb =  parseFloat(a) + parseFloat(b);
				
				gb =  parseFloat(gb).toFixed(2);
				gb = parseFloat(gb).toFixed(3);
				return (gb);
            }, 0);
            $(tgbapi.column(4).footer()).html(tgb);
			var p = api.column(5).data().reduce(function (a, b) {
                var mc =  parseFloat(a) + parseFloat(b);
				return parseFloat(mc).toFixed(2);
            }, 0);
            $(api.column(5).footer()).html(p);
			var tsc = api.column(6).data().reduce(function (a, b) {
				var sc =  parseFloat(a) + parseFloat(b);
				return parseFloat(sc).toFixed(2);
            }, 0);
            $(api.column(6).footer()).html(tsc);
      },
	
        } );
		function calculateColumnSumAndDisplayInHeader(columnIndex1, columnIndex2) {
  var columnSum1 = 0;
  var columnSum2 = 0;

  // Calculate the sum for column with index 1
  $('#rate_table tbody tr').each(function () {
    var cellValue = parseFloat($(this).find('td').eq(columnIndex1).text());

    if (!isNaN(cellValue)) {
      columnSum1 += cellValue;
    }
  });

  // Calculate the sum for column with index 2
  $('#rate_table tbody tr').each(function () {
    var cellValue = parseFloat($(this).find('td').eq(columnIndex2).text());

    if (!isNaN(cellValue)) {
      columnSum2 += cellValue;
    }
  });

  // Update the header text for column 1 with the sum
  $('#rate_table th').eq(columnIndex1).text($('#myTable th').eq(columnIndex1).text() + ' (Sum: ' + columnSum1 + ')');

  // Update the header text for column 2 with the sum
  $('#myTable th').eq(columnIndex2).text($('#myTable th').eq(columnIndex2).text() + ' (Sum: ' + columnSum2 + ')');
}
$(document).ready(function() {
// Call the function to calculate sum and update headers for columns with indices 1 and 2
//calculateColumnSumAndDisplayInHeader(5, 6);
