...
<div class="form-div">
	...
	<form action="<?php echo site_url("polish/edit_new_order/".$order['Order_Code']); ?>" method='POST' enctype="multipart/form-data" name="create_form" id="create-form-id">
		...
		<div class="form-block">
			...
			<div class="card">
				...
				<table class="table table-bordered" id="order-items-table">
					<tr>
						...
						<td>Balance Gold in Grams</td>
						<?php } ?>
						<td>NW after Polish</td>
						<td>MC (Rs)</td>
				...
				foreach($order['polish_items'] as $item): $count++; 
				
				$sub_cats=$this->Polish_model->get_subcats_by_cat($item['Category_ID']);
				$jwelleryItems=$this->Polish_model->getJwelleryItems($item['SubCategory_ID']);
				?>
				<tr id="item-row-<?php echo $count; ?>">
						...
						<td><input placeholder="NW" type="number" class="form-input input-micro-width approx_gram_class" style='width:70px !important;' name="item[<?php echo $count; ?>][approx_grams]" id="approx-grams-<?php echo $count; ?>" value="<?php echo $item['approx_grams']; ?>" oninput='calculate_total_grams(<?php echo $count; ?>)'  ></td>
						...
						<td id='total-grams-<?php echo $count; ?>'>
							<?php $total_grams_row=($item['approx_grams']+($item['approx_grams']*$item['wastage']/100));
							//+($item['approx_grams']*$item['making_charges']/100)
							echo round($total_grams_row,3); 
							
							
							
							?>
						</td>
						<td id='gold-balance-<?php echo $count; ?>'>
							<?php $gold_balance=($item['gold_balance']);
							
							?>
							<input type='text' class="form-input input-micro-width balance_gold_class" style='width:50px !important;' name='item[<?php echo $count; ?>][gold_balance]' id='gold_balance-<?php echo $count; ?>' value='<?php echo $item['gold_balance']; ?>' oninput='update_total_outstanding_grams()'>
						</td>
						<?php 
							}
						...
						$row_making_charges= $item['making_charges'] ; 
							...
							?>
							<td class=''  ><input class='form-input input-micro-width nw_after_repair' type='number' id='total-grams-<?php echo $count; ?>'  name='item[<?php echo $count; ?>][nw_after_repair]' style='width:70px !important;' oninput='calculate_total_grams(<?php echo $count; ?>)' value='<?php echo $item['nw_after_repair']; ?>' /></td>
						<td id='making-charges-<?php echo $count; ?>' class=" "><input type='text' class="form-input input-micro-width  making-charge-rs" style='width:50px !important;' name='item[<?php echo $count; ?>][making_charges]' id='making-charge-per-gram-<?php echo $count; ?>' value='<?php echo $item['making_charges']; ?>' oninput='update_total_outstanding_grams()'>
							
						</td>
						<td>
							<?php  echo ($item['receipt_file']!="") ? "<a target='_blank' href='".site_url("uploads/order_receipts/".$item['receipt_file'])."' data-lightbox='example-1' class='example-image-link'><img src='".site_url("uploads/order_receipts/".$item['receipt_file'])."' height='20px'></a>" : "" ?><div class="custom-file"><input type='file' id="orderFile<?php echo $count; ?>"  name='receipt_order_file[<?php echo $count; ?>]' class="custom-file-input" /><label class="custom-file-label" for="orderFile<?php echo $count; ?>"></label></div>
				...
				<input type="hidden" name="item-count" id="item-count" value="<?php echo count($order['polish_items']); ?>" />
			...
			<div class="card">
				...
				<div class="card-header"><strong> Cash Receipts</strong> <?php if($rate_per_gram>0): ?> <a class="form-button small-button"  onclick='add_cash("cash");'> + </a> <?php else: ?>&nbsp;<a href="<?php echo site_url("rate/browse?create_form=yes&metal_type=$metal_type"); ?>"><span style="color:red;">Add today's <?php echo $metal_type ?> rate </span> </a><?php endif;  ?>  </div>
					...
					<tr id='receipt-row-<?php echo $count; ?>'>
						...
						<td>
							<?php if($op['Payment_Method']=='cash'): ?>
								<input placeholder="Amount" type="number" class="form-input input-micro-width valid floatNumberField cash_amount_class" style='width:90px !important;' name="cash_receipt[<?php echo $count; ?>][total_amount]" id="receipt-amount-<?php echo $count; ?>" value="<?php echo $op['total_amount']; ?>" oninput="update_cash_receipt_grams;" ><input type="hidden" name="cash_receipt[<?php echo $count; ?>][Payment_Method]" id="receipt-type-<?php echo $count; ?>" value="cash">
						...
						<td><input type="button" class="form-button small-button bg-red valid" name="remove_receipt_<?php echo $count; ?>" id="remove-receipt-<?php echo $count; ?>" value="x" onclick="remove_receipt('receipt-row-<?php echo $count; ?>')" ><input type="hidden" id="total-cash-receipt-amt-<?php echo $count; ?>" name="total_cash_receipt_amt[]" value="<?php echo $op['total_amount']; ?>"></td>
					</tr>
...
<script>
		...
		workshop_options="<option value=''>-Select Workshop-</option><?php foreach($workshops as $ws): echo "<option value='".$ws['Workshop_Code']."'>".$ws['Workshop_Name']."</option>"; endforeach; ?>";
		 ...
		 item="<tr id='item-row-"+item_count+"'>"+
			 ...
			 "<td><input placeholder='NW' type='number' class='form-input input-micro-width approx_gram_class' style='width:70px !important;' name='item["+item_count+"][approx_grams]' id='approx-grams-"+item_count+"' value='0' oninput='calculate_total_grams("+item_count+")' /></td>";
				...
				"<td><div class='custom-file' ><input type='file'   id='orderFile"+item_count+"' name='receipt_order_file["+item_count+"]' class='custom-file-input' /><label class='custom-file-label' for='orderFile"+item_count+"'></label></div></td>"+
	...
	function calculate_total_grams(row_id){
		...
		gross_total=(approx_grams);
		
	//	net_total=gross_total+(gross_total*making_charge_percent/100);
	
		net_total=parseFloat(gross_total).toFixed(3);
		...
		making_charges=parseInt(making_charge_per_gram);
	...
	function update_total_outstanding_grams(){
		...
		total_grams = fixFloat(total_grams);
		$('#total-outstanding-grams').html(total_nw);
		...
		making_charges_array.each(function(){
			...
			net_making_charges = net_making_charges + extractNumericValue(making_charge);
		...
		total_cash_amount = parseFloat($('#total_cash_amount').html());
		...
		$('#total-remaining-balance').html(net_making_charges-total_cash_amount);
	...
	function add_gold(receipt_type){
		...
		if(receipt_type=="cash"){
			...
			// "<td><select class='form-select input-micro-width' name='receipt["+receipt_count+"][SubCategory_ID]' id='sub-category-deposit-"+receipt_count+"' ><option value=''>-Sub Category-</option></select></td>"+	
			 ...
			 "<td><input placeholder='Grams' type='number' class='form-input input-micro-width floatGramField' style='width:70px !important;' name='receipt["+receipt_count+"][Grams]' id='receipt-grams-"+receipt_count+"' value='0' oninput='calculate_receipt_grams("+receipt_count+");' /></td>"+
	...
	function add_cash(receipt_type){
		...
		if(receipt_type=="cash"){
			...
			amount_input="<td><input placeholder='Amount' type='number' class='form-input input-micro-width cash_amount_class' style='width:90px !important;' name='cash_receipt["+receipt_count+"][total_amount]' id='receipt-amount-"+receipt_count+"' value='0' oninput='update_cash_receipt_grams();' /><input type='hidden' name='cash_receipt["+receipt_count+"][Payment_Method]' id='receipt-type-"+receipt_count+"' value='cash' /><input type='hidden' name='cash_receipt["+receipt_count+"][rate_per_gram]' id='rate-per-gram-"+receipt_count+"' value='<?php echo $rate_per_gram; ?>'></td>";
			...
			"<td><select class='form-select input-micro-width' name='cash_receipt["+receipt_count+"][txn_type]' id='txn_type_-"+receipt_count+"' >"+category_options+"</select></td>"+
			 ...
			 "<td><input type='button' class='form-button small-button bg-red' name='remove_receipt_"+receipt_count+"' id='remove-receipt-"+receipt_count+"' value='x'  onclick='remove_receipt(\"receipt-row-"+receipt_count+"\", \"receipt-count\")'/><input type='hidden' id='total-cash-receipt-amt-"+receipt_count+"' name='total_cash_receipt_amt[]' value='0' /></td>"+	
...
//function to populate deposit record
	...
	function calculate_silver_receipt_grams(row_id){
		...
		$('#net-silver-deposit-grams-'+row_id).val(parseFloat(net_silver_receipt).toFixed(3));
